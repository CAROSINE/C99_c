...
int main()
{
    ...
    for(i=0;i<n;i++)  // input process details
    {
        ...
        completion[i] = -1;  // initialize completion time
    }
    for (i=0;i<n;i++) // sorting processes based on arrival time (initial order)
    {
       pos = i ;
       for (j = i+1; j<n; j++)
       {
    ...
    while(completed != n)
    {
        ...
        for (i = 0; i < n; i++)
        {
            if (arrival[i] <= current_time && is_completed[i] == 0)
            {
                ...
                // if burst time equal , pick the one that arrived first
