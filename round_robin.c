...
int main()
{
  ...
  int process[i], arrival[i], burst[i], remaining[i], completion[i] ;
  int waiting[i], turnaround[i];

  for(i=0; i<n; i++)
  {
    process[i] = i+1;
    printf("Enter the arrival time and burst time for process %d: ", process[i]);
